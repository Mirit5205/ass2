...
public class Paddle implements Sprite, Collidable {
    ...
    public boolean isPaddleCrossRightEdge(Point p) {
        return p.getX() + this.getCollisionRectangle().getWidth() > Game.GUI_WIDTH - Game.GUI_BLOCK_EDGE_SIZE - PADDLE_STEP;
    }
    /**
    ...
    public Velocity hit(CollisionInfo c, Velocity currentVelocity) {
        ...
        if (Math.sin(angel) == 0) {
             speed = currentVelocity.getDy() / - Math.cos(angel);
        } else {
             speed = currentVelocity.getDx() / Math.sin(angel);
        }
*/
